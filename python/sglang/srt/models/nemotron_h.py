...
class NemotronHMLPLikeDecoderLayer(nn.Module):
    """MLP half of a decoder layer, shared by the dense-MLP / MoE layers.

    A preceding Mamba / attention layer hands over its unreduced output, the
    same hand-off as between the two halves of a standard decoder layer.
    """
    ...
    def forward(
        ...
        for i in range(self.start_layer, self.end_layer):
            ...
            if i in self.layers_to_capture:
                ...
                if not layer.input_is_reduced:
                    # The layer's prepare_mlp reduces its input; capture a
                    # reduced copy.
                    captured = attn_tp_all_reduce(hidden_states.clone())
                aux_hidden_states.append(
                    captured if residual is None else captured + residual
                )
